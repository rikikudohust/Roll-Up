...
const {buildBabyjub, buildSMT} = require('circomlibjs');
 ...
 */
async function newLevelDbEmptyTree(pathDb) {

	const babyJubjub = await buildBabyjub();
	const F = babyJubjub.F;
	const db = new SMTLevelDb(pathDb, F);
	const rt = await db.getRoot();
	const smtLevelDb = await buildSMT(db, rt);
	return smtLevelDb;
}
