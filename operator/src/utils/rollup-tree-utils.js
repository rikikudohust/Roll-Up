const {
    padZeroes, arrayHexToBigInt, buildElement,
} = require("./utils");
 ...
 */
async function hashLeafValue(balance, tokenId, Ax, Ay, withdrawAddress, nonce) {
    ...
    return { leafObj, hash: poseidon(entryBigInt) };
}
 ...
 */
async function hashStateTree(balance, tokenId, Ax, Ay, nonce) {
    // Build Entry
    // element 0
    const poseidon = await buildPoseidon();
    ...
    // Get array BigInt
    const entryBigInt = arrayHexToBigInt([e0, e1, e2, e3]);
    // Object leaf
    const leafObj = {
        balance,
        tokenId,
    ...
    // Hash entry and object
    return { leafObj, elements: {e0, e1, e2, e3}, hash:await poseidon(entryBigInt) };
}
 ...
 * @param {BigInt} withdrawAddress - ethereum address
 */
async function hashExitLeafValue(id, amount, tokenId, withdrawAddress) {
    // Build Entry
    // element 0
    const poseidon = await buildPoseidon();
