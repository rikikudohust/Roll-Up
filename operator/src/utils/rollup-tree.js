...
class RollupTree {
     ...
     */
    async addId(id, balance, tokenId, Ax, Ay , nonce) {
        const resDeposit = await utils.hashStateTree(balance, tokenId, Ax, Ay, nonce);
        await this.leafDb.put(resDeposit.hash, this._toString(resDeposit.leafObj));
    ...
    async addIdExit(id, amount, tokenId, ethAddress) {
        ...
        await this.leafDb.put(resExit.hash, this._toString(resExit.leafObj));
 ...
 * @returns {RollupTree} - rollup tree class
 */
async function newLevelDbRollupTree(path) {
    const lastTreeDb = new Level(`${path}-leafs`);
    const tree = await newLevelDbEmptyTree(`${path}-tree`);
    const rollupTree = new RollupTree(lastTreeDb, tree);
    return rollupTree;
}
