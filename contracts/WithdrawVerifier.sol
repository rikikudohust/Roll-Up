...
contract WithdrawVerifier {
    ...
    function verifyingWithdrawKey() internal pure returns (VerifyingWithdrawKey memory vk) {
    ...
    function verifyWithdraw(uint[] memory input, ProofWithdraw memory proof) internal view returns (uint) {
        uint256 snark_scalar_field = 21888242871839275222246405745257275088548364400416034343698204186575808495617;
        VerifyingWithdrawKey memory vk = verifyingWithdrawKey();
    ...
    function verifyWithdrawProof(
        ...
        if (verifyWithdraw(inputValues, proof) == 0) {
            return true;
        } else {
            return false;
        }
